      ...
      <div className="mt-16 divide-y divide-deepink/10">
        {projects.map((p) => (
          <Link
            to={`/work/${p.slug}`}
            key={p.slug}
            className="group grid gap-4 py-10 sm:grid-cols-[120px_1fr_auto] sm:items-start sm:gap-8"
          >
            ...
            <Doodle
              variant="star"
              className="hidden h-5 w-5 text-rose/70 transition-transform group-hover:rotate-12 sm:block"
            />
          </Link>
        ))}
      </div>
